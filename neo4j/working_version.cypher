...
LOAD CSV WITH HEADERS FROM 'file:///houses.csv' AS house_row
    MATCH (d:district {name: house_row.district}),
        (h:house)
    WHERE
        d.name = h.district
    CREATE
...
LOAD CSV WITH HEADERS FROM 'file:///close1.csv' AS close_row
    MATCH (h:house {houseId: toInteger(close_row.house_id)}),
        (t:temple {templeId: close_row.temple_id})
    MERGE
        (h)-[r1:CLOSE_TO {distance: toFloat(close_row.distance)}]->(t);

LOAD CSV WITH HEADERS FROM 'file:///close2.csv' AS close_row
    MATCH (h:house {houseId: toInteger(close_row.house_id)}),
        (t:temple {templeId: close_row.temple_id})
    MERGE
        (h)-[r2:CLOSE_TO {distance: toFloat(close_row.distance)}]->(t);
...
LOAD CSV WITH HEADERS FROM 'file:///close3.csv' AS close_row
    MATCH (h:house {houseId: toInteger(close_row.house_id)}),
        (t:temple {templeId: close_row.temple_id})
    MERGE
        (h)-[r3:CLOSE_TO {distance: toFloat(close_row.distance)}]->(t);
